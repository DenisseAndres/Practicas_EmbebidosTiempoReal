    ...
    for (;;)
     {
        ...
        uint16_t adc_raw = adc_reader_get_raw(cfg->adc_channel); // Reemplazar con lectura real del ADC

        /* TODO  2. Escalar a duty PWM de 8 bits [0, 255] */
        uint8_t duty = adc_reader_to_duty(adc_raw); // Escalado lineal

        /* TODO 3. Aplicar brillo al LED correspondiente */
        leds_set_duty(cfg->led_channel, duty); // Reemplazar con funcion real de control de LED
        ...
        if (cycle_count >= TASK_LOG_EVERY_N) {
            ESP_LOGI(TAG, "[%s] raw= %u  duty= %u heap= %u wm= %u",
                     cfg->name, adc_raw, duty, esp_get_free_heap_size(), esp_get_minimum_free_heap_size());
            cycle_count = 0U;
        }
        ...
        vTaskDelay(pdMS_TO_TICKS(TASK_PERIOD_MS)); // Bloqueo por periodo configurado
    }
...
void tasks_create_all(void)
{
    ...
    /* TODO  Task Create Canal 1 - Prioridad 2 */
    ret = xTaskCreate(vTaskPotLED,
                      "PotLED_1",
                      TASK_STACK_SIZE_WORDS,
                      (void *)&params_ch1,
